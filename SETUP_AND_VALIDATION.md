...
For updates, extract a new package into a separate folder, review its .env.example and migration notes, then copy only your intended configuration values. Stop the previous project before starting the replacement on the same address and ports. Run docker compose pull followed by docker compose up -d --wait to use the image versions named in the new Compose file. The application package pins versioned images by manifest digest rather than latest; keep the previous Compose file and image versions for rollback. Preserve the previous package and .env for rollback.
